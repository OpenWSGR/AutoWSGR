...
`tests/unit/ui/` 和 `tests/manual/ui/` 下每个 UI 控制器对应一个子目录，目录内包含两类测试：
...
```bash
# 自动检测设备，序列运行所有测试
python tests/manual/ui/run_all_e2e.py

# 指定设备并启用调试
python tests/manual/ui/run_all_e2e.py --serial emulator-5554 --debug

# 自定义等待时间（每步 2 秒）
python tests/manual/ui/run_all_e2e.py --pause 2.0

# 指定汇总报告输出路径
python tests/manual/ui/run_all_e2e.py --output my_report.json
```
...
1. 自动搜索所有 `tests/manual/ui/*/e2e.py` 脚本
...
```
python tests/ui/<page>/e2e.py [serial] [--auto] [--debug] [--pause SECONDS]
```
...
```bash
# 交互模式，自动检测设备
python tests/manual/ui/main_page/e2e.py

# 自动模式，指定设备序列号
python tests/manual/ui/main_page/e2e.py emulator-5554 --auto

# 调试详细日志，动作间隔 2 秒
python tests/manual/ui/map_page/e2e.py --auto --debug --pause 2.0
```
...
- 若要在 CI 中运行 e2e，需显式调用 `python tests/manual/ui/<page>/e2e.py --auto`，并确保 ADB 已连接。
