...
| `MapPage` | `map_page.py` | 5 面板 + OCR 地图识别 | 面板切换、章节导航、地图选择 |
| `BattlePreparationPage` | `battle_preparation.py` | 舰队+面板联合检测 | 舰队选择、面板切换、开始出征 |
...
- 所有坐标为 **相对值** (0.0–1.0)
- 由旧代码 960×540 绝对坐标换算: `rx = x / 960`, `ry = y / 540`
- 分为两类:
  - **探测坐标** — 采样像素颜色用于状态检测
  - **点击坐标** — 执行操作
...
| 绝对坐标 (960×540) | 相对坐标 (0.0–1.0) | 分辨率无关 |
...
├── main_page.py              # ✅ 主页面
├── map_page.py               # ✅ 地图页面 (含 5 个面板)
├── battle_preparation.py     # ✅ 出征准备
├── sidebar_page.py           # ✅ 侧边栏
