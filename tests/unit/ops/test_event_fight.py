...
class TestInitMapCode:
    ...
    def test_non_event_chapter_empty_map_code(self) -> None:
        ctx = _make_ctx()
        plan = _make_plan(chapter='X', map_id=5)
        runner = EventFightRunner(ctx, plan)
        assert runner._map_code == ''

    def test_defaults_none_empty_map_code(self) -> None:
        ctx = _make_ctx()
...
class TestInitMode:
    ...
    def test_normal_corrected_to_event_with_event_chapter(self) -> None:
        ctx = _make_ctx()
        plan = _make_plan(mode=CombatMode.NORMAL, chapter='H')
        runner = EventFightRunner(ctx, plan)
        assert runner._plan.mode == CombatMode.EVENT

    def test_event_unchanged(self) -> None:
        ctx = _make_ctx()
        plan = _make_plan(mode=CombatMode.EVENT, chapter='H')
        runner = EventFightRunner(ctx, plan)
        assert runner._plan.mode == CombatMode.EVENT

    def test_normal_unchanged_without_event_chapter(self) -> None:
        ctx = _make_ctx()
        plan = _make_plan(mode=CombatMode.NORMAL)
        runner = EventFightRunner(ctx, plan)
        assert runner._plan.mode == CombatMode.NORMAL
...
class TestInitFleetId:
    ...
    def test_default_none(self) -> None:
        ...
        assert runner._fleet_id is None
...
class TestRunForTimes:
    ...
    def test_gap_sleep(self) -> None:
        ctx = _make_ctx()
        plan = _make_plan(chapter='H', map_id=1)
        runner = EventFightRunner(ctx, plan)
        with (
            patch.object(
                runner, 'run', return_value=CombatResult(flag=ConditionFlag.OPERATION_SUCCESS)
            ),
            patch('autowsgr.ops.normal_fight.time.sleep') as mock_sleep,
        ):
...
class TestRunEventFight:
    ...
    def test_calls_runner(self, mock_cls: MagicMock) -> None:
        ...
        assert mock_cls.call_args.args[0] is ctx
        assert mock_cls.call_args.args[1] is plan
        assert mock_cls.call_args.kwargs['map_code'] == 'H3'
        assert mock_cls.call_args.kwargs['entrance'] is None
