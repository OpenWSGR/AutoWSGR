"""出征准备页面常量 — 坐标与颜色参考值。

从 ``battle_preparation.py`` 中分离的纯数据常量。
"""

from __future__ import annotations

from autowsgr.vision import Color


# ═══════════════════════════════════════════════════════════════════════════════
# 选中态参考颜色 (RGB)
# ═══════════════════════════════════════════════════════════════════════════════

FLEET_ACTIVE = Color.of(16, 133, 228)
"""舰队标签选中态颜色 — 明亮蓝色。"""

PANEL_ACTIVE = Color.of(30, 139, 240)
"""面板标签选中态颜色 — 明亮蓝色。"""

AUTO_SUPPLY_ON = Color.of(13, 140, 233)
"""自动补给启用态颜色 — 蓝色勾选框。"""

STATE_TOLERANCE = 30.0
"""状态检测颜色容差。"""


# ═══════════════════════════════════════════════════════════════════════════════
# 探测坐标 — 采样颜色判断状态
# ═══════════════════════════════════════════════════════════════════════════════

FLEET_PROBE: dict[int, tuple[float, float]] = {
    1: (0.0750, 0.1731),
    2: (0.1974, 0.1778),
    3: (0.3271, 0.1694),
    4: (0.4479, 0.1713),
}
"""舰队标签探测点。选中项探测颜色 ≈ (16, 133, 228)。"""

SUPPORT_PROBE: tuple[float, float] = (0.6521, 0.1843)
"""战役支援探测点。"""

AUTO_SUPPLY_PROBE: tuple[float, float] = (0.0552, 0.9343)
"""自动补给探测点。启用态探测颜色 ≈ (13, 140, 233)。"""


# ═══════════════════════════════════════════════════════════════════════════════
# 点击坐标 — 执行操作
# ═══════════════════════════════════════════════════════════════════════════════

CLICK_BACK: tuple[float, float] = (0.022, 0.058)
"""回退按钮 (◁)。"""

CLICK_FLEET: dict[int, tuple[float, float]] = {
    1: (0.088, 0.170),
    2: (0.197, 0.170),
    3: (0.327, 0.170),
    4: (0.448, 0.170),
}
"""舰队标签点击位置。"""

CLICK_SUPPORT: tuple[float, float] = (0.640, 0.180)
"""战役支援点击位置。"""

CLICK_AUTO_SUPPLY: tuple[float, float] = (0.095, 0.935)
"""自动补给复选框点击位置。"""

CLICK_START_BATTLE: tuple[float, float] = (0.850, 0.935)
"""「开始出征」按钮点击位置。"""

CLICK_SHIP_SLOT: dict[int, tuple[float, float]] = {
    1: (0.1146, 0.4630),
    2: (0.2292, 0.4630),
    3: (0.3438, 0.4630),
    4: (0.4583, 0.4630),
    5: (0.5729, 0.4630),
    6: (0.6875, 0.4630),
}
"""6 个舰船槽位的点击坐标 (1-indexed)。"""


# ═══════════════════════════════════════════════════════════════════════════════
# 血量检测
# ═══════════════════════════════════════════════════════════════════════════════

BLOOD_BAR_PROBE: dict[int, tuple[float, float]] = {
    1: (0.0583, 0.5963),
    2: (0.1750, 0.5963),
    3: (0.2917, 0.5963),
    4: (0.4083, 0.5963),
    5: (0.5250, 0.5963),
    6: (0.6417, 0.5963),
}
"""6 个舰船血条探测点 (1-indexed)。"""

BLOOD_GREEN = Color.of(69, 162, 117)
"""绿血 — 无损/轻微。"""
BLOOD_YELLOW = Color.of(246, 184, 51)
"""黄血 — 中破。"""
BLOOD_RED = Color.of(230, 58, 89)
"""红血 — 大破。"""
BLOOD_BLACK = Color.of(96, 91, 92)
"""黑色 — 维修中。"""
BLOOD_EMPTY = Color.of(43, 87, 112)
"""蓝色/空 — 无舰船。"""

BLOOD_TOLERANCE = 40.0
"""血条颜色检测容差 (需要比普通像素更宽松)。"""


# ── 战斗结算页血量检测 ──
# 旧代码 BLOOD_BAR_POSITION[1]: 绝对坐标 (60, 142), (60, 217), ... 基于 960×540
# 转换为归一化坐标

RESULT_BLOOD_BAR_PROBE: dict[int, tuple[float, float]] = {
    1: (60 / 960, 142 / 540),
    2: (60 / 960, 217 / 540),
    3: (60 / 960, 292 / 540),
    4: (60 / 960, 367 / 540),
    5: (60 / 960, 442 / 540),
    6: (60 / 960, 517 / 540),
}
"""战斗结算页 6 个舰船血条探测点 (1-indexed)。"""

RESULT_BLOOD_GREEN = Color.of(70, 182, 88)
"""结算页绿血 — 无损/轻微。"""
RESULT_BLOOD_YELLOW = Color.of(238, 186, 64)
"""结算页黄血 — 中破。"""
RESULT_BLOOD_RED = Color.of(166, 33, 3)
"""结算页红血 — 大破。"""


# ═══════════════════════════════════════════════════════════════════════════════
# 战役支援颜色 (RGB)
# ═══════════════════════════════════════════════════════════════════════════════

SUPPORT_ENABLE = Color.of(228, 182, 60)
"""战役支援启用 — 黄色。"""
SUPPORT_DISABLE = Color.of(44, 142, 239)
"""战役支援禁用 — 蓝色。"""
SUPPORT_EXHAUSTED = Color.of(143, 145, 154)
"""战役支援次数用尽 — 灰色。"""
