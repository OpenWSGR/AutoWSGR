...
class Launcher:
    timer: Timer
    daily_operation: DailyAutomationConfig
    ...
    def __init__(self, config: UserConfig) -> None:
        ...
        Returns:
            Timer: 该模拟器的记录器
        """
        self.daily_operation = config.daily_automation
        self.decise_battle = config.decisive_battle
        ...
        self.timer.port.factory = BuildManager(timer)

    def run_daily_automation(self) -> None:
        """运行日常自动化脚本"""
        if self.daily_operation is not None:
            daily_operation = DailyOperation(self.timer, self.daily_operation)
            daily_operation.run()
        else:
    ...
    def run_decisive_battle(self, times: int = 1) -> None:
        """运行决战脚本"""
        if self.decise_battle is not None:
            decisive_battle = DecisiveBattle(self.timer, self.decise_battle)
