...
运行方式::
    ...
    # 交互模式 (默认)
    python tests/manual/ui/event_page/e2e.py

    # 自动执行
    python tests/manual/ui/event_page/e2e.py --auto

    # 指定设备
    python tests/manual/ui/event_page/e2e.py emulator-5554 --auto --debug
...
def _try_enter_node(event_page: BaseEventPage, node_id: int) -> None:
    ...
    try:
        event_page._enter_node(node_id)
